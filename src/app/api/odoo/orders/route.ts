...
export async function POST(req: NextRequest) {
  try {
    ...
    const saleId = await client.createSaleOrderFromWebsiteOrder(order, body.partner);
    ...
    const webUrl = host ? `${host}/web#model=sale.order&id=${saleId}&view_type=form` : undefined;

    return jsonResponse(
      successResponse({ saleId, quotationNumber: id, confirmed, webUrl }, confirmed ? 'Sale order created and confirmed' : body.autoConfirm ? 'Quotation created (confirmation attempted)' : 'Quotation created')
    );
  } catch (err: any) {
    return jsonResponse(errorResponse(err?.message || 'Failed to create order in Odoo'), 500);
  }
}
...
export async function GET() {
  try {
    ...
    if (!configured) return jsonResponse(successResponse({ configured, hasSale: false }, 'Odoo not configured'));
    ...
    const hasSale = await client.modelExists('sale.order').catch(() => false);
    const productCount = await client.searchCount('product.product', [['sale_ok', '=', true]]).catch(() => 0);

    return jsonResponse(successResponse({ configured, ping, hasSale, productCount }, 'Odoo orders diagnostics'));
  } catch (err: any) {
    return jsonResponse(errorResponse(err?.message || 'Failed to get diagnostics'), 500);
  }
}
