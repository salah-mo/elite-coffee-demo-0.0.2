...
export async function POST(req: NextRequest) {
  try {
    ...
    const item: OrderItem = {
      ...
      unitPrice,
      totalPrice: subtotal,
      menuItem: { id: menuItemId, name: itemName, description: itemName, price: unitPrice, category: 'test', subCategory: 'test', images: [], featured: false, available: true, allergens: [], sizes: [], flavors: [], toppings: [] },
    };
    ...
    return jsonResponse(successResponse({ saleId, order, webUrl }, 'Sale order created'));
  } catch (err: any) {
    return jsonResponse(errorResponse(err?.message || 'Failed to create sale order'), 500);
  }
}
