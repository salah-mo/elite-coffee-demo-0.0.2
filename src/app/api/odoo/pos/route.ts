...
export async function GET(_request: NextRequest) {
  try {
    ...
    if (!configured) {
      return jsonResponse(successResponse({ configured, hasPos: false }, 'Odoo not configured'));
    }
    ...
    if (!hasPos) {
      return jsonResponse(successResponse({ configured, hasPos, configs: [], ping }, "POS module not available"));
    }
    ...
    const message = err instanceof Error ? err.message : 'Failed to fetch POS diagnostics';
