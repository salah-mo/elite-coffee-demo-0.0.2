  ...
  async function runPostTest() {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/api/odoo/test`,
        {
          method: "POST",
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <p className="text-sm text-gray-600 mb-4">
        This page calls both <code>GET /api/odoo/test</code> (non-mutating ping)
        and <code>POST /api/odoo/test</code> (find or create a partner) and
        prints the raw JSON responses.
      </p>
      ...
      <div className="mt-4 text-sm text-gray-600">
        <p>
          Tip: Ensure Odoo env vars are set in <code>.env</code>: ODOO_HOST,
